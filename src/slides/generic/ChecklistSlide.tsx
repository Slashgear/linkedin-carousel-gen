...
export interface ChecklistSlideProps {
  title: string;
  items: ChecklistItem[];
  badge?: string;
  logoUrl?: string;
...
export function ChecklistSlide({
  ...
  const renderText = (item: ChecklistItem) => {
          ...
          >
            <div
              style={{
                ...
                height: 40,
                borderRadius: 20,
                backgroundColor: item.checked !== false ? theme.colors.accent : "transparent",
                border: item.checked !== false ? "none" : `2px solid ${theme.colors.textMuted}`,
                marginRight: theme.spacing.md,
                fontSize: theme.typography.fontSizeSm,
                color: item.checked !== false ? "#000" : theme.colors.textMuted,
              }}
            >
              {item.checked !== false ? "✓" : ""}
            </div>
            <span
              style={{
                fontSize: 28,
                flex: 1,
                textDecoration: item.checked === false ? "none" : "none",
                opacity: item.checked === false ? 0.6 : 1,
              }}
            >
              {renderText(item)}
            </span>
